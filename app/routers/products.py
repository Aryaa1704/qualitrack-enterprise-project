...
async def create_product(request: Request, db: Annotated[Session, Depends(get_db)], current_user: Annotated[User, Depends(require_role(ROLE_ADMIN))]) -> Product | RedirectResponse:
...
def list_products(request: Request, db: Annotated[Session, Depends(get_db)], current_user: Annotated[User, Depends(require_role(ROLE_ADMIN, ROLE_QUALITY_MANAGER, ROLE_INSPECTOR))], search: str | None = None, category: str | None = None, status_filter: str | None = None, page: int = 1, per_page: int = 10) -> ProductList | HTMLResponse:
...
def get_product(product_id: int, request: Request, db: Annotated[Session, Depends(get_db)], current_user: Annotated[User, Depends(require_role(ROLE_ADMIN, ROLE_QUALITY_MANAGER, ROLE_INSPECTOR))]) -> Product | HTMLResponse:
...
async def update_product(product_id: int, request: Request, db: Annotated[Session, Depends(get_db)], current_user: Annotated[User, Depends(require_role(ROLE_ADMIN))]) -> Product:
    product = _get_product_or_404(db, product_id)
...
def delete_product(product_id: int, db: Annotated[Session, Depends(get_db)], current_user: Annotated[User, Depends(require_role(ROLE_ADMIN))]) -> Product:
    product = _get_product_or_404(db, product_id)
